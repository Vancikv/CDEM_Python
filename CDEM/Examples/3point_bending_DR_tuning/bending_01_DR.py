...
table['d_t'] = 4 * [0.001]
table['density'] = 4 * [1.]
table['alfaC'] = 4 * [i * ((25.*1.) ** 0.5) for i in [8., 14., 28., 50.]]
table['stiffness_factor'] = [10., 20., 50., 100.]
n_iter = 100000
...
for dt, ro, alfaC, sfact in zip(table['d_t'], table['density'], table['alfaC'], table['stiffness_factor']):
    ...
    dom.C /= sfact
f = open(get_outfile(['Dropbox', 'CDEM', '3point_bending_DR_tuning'], '3point_bending_DR_sfactconverg.txt'), 'w')
