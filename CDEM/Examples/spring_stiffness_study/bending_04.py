...
Beam from bending01 with an uneven mesh.
'''
...
dom2 = get_uneven_mesh(ni0=3, nj=16, lx=100., ly=30., stencil_arr=['0', '0', '0', 'f', '0', '0', '0', '0', '0', '0', '0', '0', 'c', '0', '0', '0'],
                             E=10000., nu=0.2, density=1.,
                             thickness=1., alfaC=1., stiffdef='isoparametric',
                             spring_stiffness_factor=1., domtype='CDEMStatic',
                             load=[((45.0, 55.0), (29.9, 30.1), [0., -16.66])],
...
dom1.plot(magnitude=50., savefile=get_outfile(['Dropbox', 'CDEM', 'spring_stiffness_study'], 'beam04.png'), show=False)
...
f = open(get_outfile(['Dropbox', 'CDEM', 'spring_stiffness_study'], 'beam04_stiffness_variation.txt'), 'w')
for i in table['Stiffness factor order']:
    ...
    dom2.solve()
    # dom2.plot(magnitude=50., savefile='c:\\Users\\Werner\\Dropbox\\CDEM\\spring_stiffness_study\\bending04\\stiff_fact_%d.png' % i, show=False)
