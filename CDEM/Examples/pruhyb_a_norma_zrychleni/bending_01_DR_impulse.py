...
dom.save_to_text_file(get_outfile(['Dropbox', 'CDEM', 'Cpp_data', '3pb'], '3pb.txt'))
'''
C_def = dom.C
...
table['d_t'] = [0.001,0.001,0.001,0.0001]
table['density'] = 4 * [1.]
table['alfaC'] = 4 * [0.]
table['stiffness_factor'] = 4*[10.]
table['n_iter'] = [500,1000,2000,20000]
...
for dt, ro, alfaC, sfact, n_iter in zip(table['d_t'], table['density'], table['alfaC'], table['stiffness_factor'], table['n_iter']):
    ...
    dom.C *= sfact
    dom.solve(dt=dt, tol=1e-16, maxiter=n_iter, node_to_plot=59, savefile=get_outfile(['Dropbox', 'CDEM', '3point_bending_DR_plots_impulse'], 'graph_dt=%.3f_niter=%.1f.png' % (dt, n_iter)),
              savetextfile=get_outfile(['Dropbox', 'CDEM', '3point_bending_DR_plots_impulse'], 'textgraph_dt=%.3f_niter=%.1f.txt' % (dt, n_iter)),show=False)
    C_min = min([min([el.C[i, i] for i in range(el.C.shape[0])]) for el in dom.elements])
    C_max = max([np.max(el.C) for el in dom.elements])
    dom.plot(magnitude=100., savefile=get_outfile(['Dropbox', 'CDEM', '3point_bending_DR_plots_impulse'], 'deformed_dt=%.3f_niter=%.1f.png' % (dt, n_iter)), show=False)
    ...
    dom.C /= sfact
f = open(get_outfile(['Dropbox', 'CDEM', '3point_bending_DR_plots_impulse'], '3point_bending_DR_plots_impulse.txt'), 'w')
