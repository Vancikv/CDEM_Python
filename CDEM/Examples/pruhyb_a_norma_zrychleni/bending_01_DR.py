...
table['density'] = 5 * [1.]
table['alfaC'] = 5 * [0.]
table['stiffness_factor'] = 5*[10.]
table['n_iter'] = [2000,4000,8000]
...
for dt, ro, alfaC, sfact, n_iter in zip(table['d_t'], table['density'], table['alfaC'], table['stiffness_factor'], table['n_iter']):
    ...
    dom.C *= sfact
    dom.solve(dt=dt, t_load=1.,t_max=4.,tol=1e-16, maxiter=n_iter, node_to_plot=59, savefile=get_outfile(['Dropbox', 'CDEM', '3point_bending_DR_plots'], 'graph_dt=%.3f_niter=%.1f.png' % (dt, n_iter)), 
              savetextfile=get_outfile(['Dropbox', 'CDEM', '3point_bending_DR_plots'], 'textgraph_dt=%.3f_niter=%.1f.txt' % (dt, n_iter)),show=False)
    C_min = min([min([el.C[i, i] for i in range(el.C.shape[0])]) for el in dom.elements])
    C_max = max([np.max(el.C) for el in dom.elements])
    dom.plot(magnitude=100., savefile=get_outfile(['Dropbox', 'CDEM', '3point_bending_DR_plots'], 'deformed_dt=%.3f_niter=%.1f.png' % (dt, n_iter)), show=False)
    ...
    dom.C /= sfact
f = open(get_outfile(['Dropbox', 'CDEM', '3point_bending_DR_plots'], '3point_bending_DR_plots.txt'), 'w')
