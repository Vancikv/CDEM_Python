...
table['d_t'] = [recommended_step/(2.**i) for i in range(7)]
table['density'] = 7 * [1.]
table['alfaC'] = 7 * [0.]
table['stiffness_factor'] = 7*[10.]
n_iter = 1000
...
for dt, ro, alfaC, sfact in zip(table['d_t'], table['density'], table['alfaC'], table['stiffness_factor']):
    ...
    dom.C *= sfact
    dom.solve(dt=dt, tol=1e-16, maxiter=n_iter, node_to_plot=59, savefile=get_outfile(['Dropbox', 'CDEM', '3point_bending_DR_time_step'], 'graph_ro=%.1f_dt=%.3f_alfaC=%f_stifffact=%.1f.png' % (ro, dt, alfaC, sfact)), show=False)
    C_min = min([min([el.C[i, i] for i in range(el.C.shape[0])]) for el in dom.elements])
    C_max = max([np.max(el.C) for el in dom.elements])
    dom.plot(magnitude=100., savefile=get_outfile(['Dropbox', 'CDEM', '3point_bending_DR_time_step'], 'deformed_ro=%.1f_dt=%.3f_alfaC=%f_stifffact=%.1f.png' % (ro, dt, alfaC, sfact)), show=False)
    ...
    dom.C /= sfact
f = open(get_outfile(['Dropbox', 'CDEM', '3point_bending_DR_time_step'], '3point_bending_DR_timestep.txt'), 'w')
